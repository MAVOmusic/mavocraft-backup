...
## 2026-09-09 v3.0.1 — boot-log verify round: EXP_BOTTLE/EMPTY_MAP/TERRA_COTTA fixed + read-only zip fix
- **User boot log (deployed 3.0.0 pack):** 59 plugins, ALL 46 MAVO at 3.0.0, no stack traces, Done 19.6s. VERIFIED OK: Guide "config v19 -> v22", Enchants "mining drop 1=0.1%/2=0.05%/3=0.01% (tiers 1-3), charge table 10 levels", Miniboss "15 boss type(s), 2 arena(s)", Crates key rates, all enabled.
- **REAL BUG 1 (ChestHunt):** `EXP_BOTTLE` is not a Material -> WARN "loot entry EXP_BOTTLE is not a valid item - skipped", pool 19. Fixed -> `EXPERIENCE_BOTTLE` (pool 20 again).
- **REAL BUG 2 (Crafting):** boot "98 beginner recipe(s)" - two recipes silently skipped: `result: EMPTY_MAP` (Material is `MAP`) and `result: TERRA_COTTA` (`TERRACOTTA`). Fixed -> 100.
- **Version rule applied:** MAVOChestHunt/Crafting/Guide 3.0.0 -> 3.0.1 (pom + finalName + plugin.yml). Guide config v22 -> v23 + What's New v23 (syncConfig will auto-upgrade live v22 -> v23 at boot).
- **Read-only complaint (user):** "12 random files read only, no permission to change" while uploading from MAVOcraft-backup.zip -> zip entries were `-rw-r--r--` (Git stores 100644, workspace 644 - fine). ROOT CAUSE assumed external (extractor/PebbleHost panel preserve mode) - FIX on our side: MAVOcraft-backup.zip regenerated with Python zipfile setting external attrs to `0o666` (rw-rw-rw-) so EVERY extracted jar/config is writable by all users; directories 0777. Jar files in repo remain 100644 in git (standard) - only the zip is made relaxant.
- **Deploy:** MAVOCrafting-3.0.1.jar + MAVOChestHunt-3.0.1.jar + MAVOGuide-3.0.1.jar (pack: 18_/24_/33_ -> 3.0.1); restart; boot should show "100 beginner recipe(s)" + ChestHunt "pool 20 item type(s)" + Guide "config v22 -> v23".
- **Zip (v3.0.1 final):** MAVOcraft-backup.zip rebuilt (95,076,580 B) with Python zipfile: every file external attr `0o666`, dirs `0o777`. VERIFIED: zipinfo shows all entries `rw-rw-rw-`; real extraction (unzip -d) gives `-rw-rw-rw-` on 100% of files (0 read-only) - the panel upload/overwrite problem is fixed. SHA `502158b220b04a0954c350d6ec1266787842da2b24c800fbaadeaa1ba86054c9` (also in MAVOcraft-backup.sha256). Pack 3.0.0/ refreshed: 43 MAVO @3.0.0 + ChestHunt/Crafting/Guide @3.0.1 (18_/24_/33_), MANIFEST + SHA256SUMS (59) + README updated.

## 2026-09-09 v3.0.2 — boot-log round 2: LIVE configs were the last bug + LuckyCoins pool fixed
- **User's 3.0.1 boot log analysis:** 59 plugins, "Done (19.073s)", 46 MAVO listed (42x 3.0.0 + 3x 3.0.1). GREEN: Guide "v22 -> v23", Professions/Tavern/PortalRoom/EconomyShopGUI/AuctionHouse/etc. all okay. **2 RED lines STILL there**: `[MAVOChestHunt] loot entry EXP_BOTTLE is not a valid item - skipped` (pool 19) and `MAVOCrafting v3.0.1 enabled ... 98 beginner recipe(s)`.
- **ROOT CAUSE (important):** the 3.0.1 fix changed the BUNDLED config inside the jars, but both plugins read the EXISTING `plugins/<Plugin>/config.yml` (Crafting's 3.0.0 upgrade had already written the 100-recipe list with EMPTY_MAP/TERRA_COTTA into it; ChestHunt's live config had EXP_BOTTLE). Jar-only fixes can never heal an existing config -> **3.0.2 adds self-healing code**: ChestHunt renames legacy loot keys in config.yml on boot (logs "3.0.2: repaired 1 legacy item name(s) ... EXP_BOTTLE -> EXPERIENCE_BOTTLE"); Crafting regex-replaces legacy names in every string leaf of config.yml (results + ingredients + anything else) and saves (logs "repaired 2 legacy material name(s) ... EMPTY_MAP->MAP TERRA_COTTA->TERRACOTTA"). `YamlConfiguration` param needed (ConfigurationSection has no save(File)) - first CI compile failure (run 34304754699), fixed, run 34305022847 success.
- **LuckyCoins bonus bug (found by auditing ALL bundled resources against the REAL Paper 26.2 Material enum, fetched from Paper commit bb09b43 via gh raw):** well-pool.txt had 4 stale names -> silently skipped every boot ("1266 sellable items (5 skipped)"): CHAIN (-> IRON_CHAIN, 26.x renamed), DISC_FRAGMENT (-> DISC_FRAGMENT_5 - duplicate lines dropped), FIREWORK (-> FIREWORK_ROCKET - duplicate lines dropped), SCUTE (-> TURTLE_SCUTE). DRAGON_EGG stays skipped by design (isHardBanned). 1269 lines total -> boot expected "1268 sellable items (1 skipped)".
- **Audit method (reusable):** `gh api repos/PaperMC/Paper/contents/paper-api/src/main/java/org/bukkit/Material.java?ref=<commit>` (commit from server boot line `26.2-119-main@bb09b43`), extract enum constants, diff against every `[A-Z][A-Z0-9_]+` token in all resource files. Only 4 invalid names existed (LuckyCoins) + the 3 already fixed in 3.0.1. Crafting bundled config verified: exactly 100 beginner recipes, 0 invalid results/ingredients.
- **Versions now:** MAVOChestHunt/Crafting/Guide/LuckyCoins = 3.0.2 (pom finalName + plugin.yml); Guide config 23 -> 24 + What's New v24 (icon IRON_CHAIN). Pack = 42 MAVO @3.0.0 + 4 @3.0.2 + 13 third-party. Boot expectations: ChestHunt "pool 20 item type(s)" NO warn; Crafting "100 beginner recipe(s)"; LuckyCoins "1268 sellable items (1 skipped)"; Guide "v23 -> v24".
- **Deliverable:** MAVOCRAFT-CHECKLIST-3.0.2.md - 74 numbered checks, EVERY one with OP side + player side (12 deploy + 58 plugin checks (46 MAVO + 12 rows covering 13 third-party) + 4 restart/persistence). Known-fine lines documented (Essentials unsupported server version, ClaimChunk Economy not enabled, CoreProtect dev branch, TAB hint, Professions Sleeper note, BlueMap manual save).
- **Zip (v3.0.2 final):** MAVOcraft-backup.zip rebuilt 95,089,033 B. NOTE (corrected 2026-09-09 v3.0.3): the SHIPPED v3.0.2 zip actually contained `-rw-r--r--` entries - the 0666 attrs from the 3.0.1 zip were lost when it was regenerated at c62a2ae, so the old "0 read-only" claim was wrong. Re-fixed + re-verified in v3.0.3 (below). SHA `93c1f658d20dbac1c851fc8967cf41515b3d0790c1e82955e9ffbf3f753603c3` (in MAVOcraft-backup.sha256).

## 2026-09-09 v3.0.3 — user playtest round: boss economy exploit + /craft wrong recipes
- **User (2nd playtest):** bosses drop "crazy" coins/keys (CRIMSON BEHEMOTH 100% 90,000 + 50% 4x LC + 45% 1x Mythic key, HP 1600; SWAMP WITCH LORD 50k) -> exploit chance; "Bosses must be really hard or crates properly adjusted". Also /craft preview for white bed showed ORANGE_BED + WHITE_DYE (a dyeing recipe), not the real one; "check all recipes to match the starter mind".
- **1) Miniboss rebalance (3.0.3):** old table = 1.26M coins total across 15 bosses, 3-6x LC @50-60%, 1-3 keys @35-70%, HP 800-2400, damage = vanilla (only speed x1.15). NEW: HP x2 (1600-4800), ATTACK_DAMAGE x1.5 (new config `boss-damage-multiplier`, code applies Attribute.ATTACK_DAMAGE), coins ~x0.1 (5k-15k, sum 126k), lucky 1-2x @30-35%, ONE key @ rare 20-25% / mythic 10-12%. `bosses-version: 2` forces live configs to take the new table (mergeBossRoster replaces whole section - live-tuned old values must not survive). Boot logs "3.0.3: boss table replaced...".
- **2) /craft recipes (3.0.3):** root cause of wrong preview = openRecipe looked up Bukkit.getRecipesFor(result) and took the FIRST shaped recipe - for white bed that's the dyeing/alt recipe (orange bed + white dye), not the real one. FIX: generated the whole 100-entry beginner list from vanilla 26.1 recipe data (PrismarineJS/minecraft-data data/pc/26.1/recipes.json + items.json fetched via gh api) - every entry now has correct ingredients, vanilla output count and a `layout` (exact 3x3 grid, AIR = empty). openRecipe renders from layout (registry only a fallback). Config upgrade `recipes-version` 3 -> 4 replaces old lists at boot ("v3.0.3: beginner recipes re-verified..."). Audit found real errors: oak_fence_gate (planks/sticks swapped), glass + terracotta (smelted, NOT crafted -> replaced with glass_bottle/glowstone entries), and ~60 wrong output counts (cookies 8, panes 16, doors 3, ladders 3, slabs 6, sticks 4, torches 4...). All 100 verified vs Paper 26.2 enum (0 invalid) + 26.1 vanilla recipes (0 mismatches after regen).
- **Versions now:** Crafting/Guide/Miniboss = 3.0.3 (pom+plugin.yml); Guide config v25 (What's New v25, icon IRON_SWORD; miniboss feature page + old v21 note updated). Pack next: 41 @3.0.0 + ChestHunt/LuckyCoins @3.0.2 + Crafting/Guide/Miniboss @3.0.3.
- **Pack + zip (v3.0.3 final):** 3.0.0/ rebuilt from the b66708b CI jars (buildpack: 59 jars MANIFEST verified, SHA256SUMS 59 lines, plugin.yml name+version vs file name all match). Pack = 41 MAVO @3.0.0 + ChestHunt/LuckyCoins @3.0.2 + Crafting/Guide/Miniboss @3.0.3 + 13 third-party; superseded root/jars copies of the three bumped plugins deleted. MAVOcraft-backup.zip regenerated with rebuildzip.py: every file entry 0666, dirs 0777 - zipinfo 434 files + 3 dirs, 0 non-writable entries; real extraction (unzip -d) = 0 read-only files + 0 read-only dirs; `unzip -t` clean. SHA in MAVOcraft-backup.sha256 (also in the finalize commit).

## 2026-09-09 v3.0.4 — REAL BUG from the 3.0.3 boot log: boss table upgrade never landed
- **Boot log (user, 3.0.3 jars live):** ALL green EXCEPT one thing - NO "[MAVOMiniboss] 3.0.3: boss table replaced" line, while Crafting DID log "re-verified" and Guide "v23 -> v25". Root cause: onEnable() ran `getConfig().options().copyDefaults(true); saveConfig()` BEFORE `mergeBossRoster()`; the bundled top-level `bosses-version: 2` key was seeded into the old disk config first, making the merge check `2 < 2` false - the OLD table (90k coins, 45% mythic) survived while claiming v2; boss-damage-multiplier 1.5 WAS seeded (so only the damage half applied).
- **Fix (3.0.4):** mergeBossRoster() now runs BEFORE copyDefaults (reads the raw disk file), version check `< 3` + bundled `bosses-version: 3` (gen 3 re-forces ANY config a 3.0.3 boot could have touched, then sticks - 2nd restart logs nothing). Log: "3.0.4: boss table replaced - hard hunts (HP x2, damage x1.5), coins/key drops scaled down to event-fair values." Guide config 25 -> 26 + What's New v26. Versions: MAVOMiniboss/MAVOGuide = 3.0.4 (pom+plugin.yml); Crafting stays 3.0.3.
- **Other boot-log notes (all non-errors):** Essentials "unsupported server version" third-party ERROR (works, always there); ClaimChunk "Economy not enabled"; CoreProtect "Development branch"; TAB hint; BlueMap manual-save WARN; EconomyShopGUI "Failed to automatically find compatible spawner provider... using default" (INFO, shops load fine); MAVOSeasonal "0 season(s) defined" - live-configs snapshot HAS 3 seasons, so the server's current config lost them (user action, not a plugin bug; restore from 3.0.0/live-configs/MAVOSeasonal/config.yml if wanted); Tavern/PersonalVault "?" = ⛃ currency glyph not rendering in the panel console only; MAVOQuests "v1.4"/ChunkBorders "v1.2"/Hud "2.2"/Casino "1.2.4"/Tavern "1.4"/CommunityGoals "v1.1"/PersonalVault "1.0.0"/AuctionHouse "1.1.0" = internal log strings, plugin.yml all 3.0.0 (5 source-built legacy strings cosmetic, could sync later).
- **Deliverable:** MAVOCRAFT-CHECKLIST-3.0.4.md (10 OP+player checks). Pack next: 41 @3.0.0 + ChestHunt/LuckyCoins @3.0.2 + Crafting @3.0.3 + Guide/Miniboss @3.0.4.
- **BOOT VERIFIED (user log 03:54:36, 3.0.4 deploy):** `[MAVOMiniboss] 3.0.4: boss table replaced - hard hunts (HP x2, damage x1.5), coins/key drops scaled down to event-fair values.` exactly once + `MAVOMiniboss v3.0.4 enabled - 15 boss type(s), 2 arena(s)`; `Guide config v25 -> v26`; Crafting `100 beginner recipe(s)` with NO repeat "re-verified"; ChestHunt pool 20 (no repair line = already healed); LuckyCoins 1268+1; 59 plugins, no stack traces, Done 19.679s. 3.0.4 fully green.
## 2026-09-10 FIX ROUND 3.0.5 — exploit audit fixes (13 jars)
- **Audit:** full source audit `MAVOCRAFT-SECURITY-AUDIT.md` + complete text checklists `CHECKLIST-OP.txt` (173 rows), `CHECKLIST-PLAYER-A.txt` (162) + `-B.txt` (140) covering every command + ATTACK rows. Found: 1 critical, 1 high, 5 medium, 4 low.
- **CRIT-01 ChestShops:** buyers paid and got NOTHING (no addItem in buy()). Fixed: buyer receives exact taken stacks (overflow drops), stock counts/takes ONLY isSimilar-to-reference stacks (reference locked at creation, persisted in data.yml; pre-3.0.5 shops fall back to plain-only so enchanted stock is never eaten cheaply), Buy-buttons-only clicks, refund if stock vanished mid-buy.
- **HIGH-02 Miniboss:** `/miniboss broadcast` had no perm/cooldown and SPAWNED a boss (unlimited farm). Replaced by user-spec schedule: 1/side, 10:00 spawn both sides, 7:00 vanish (no loot), slain side rests till next 10:00 (persisted schedule-state), spawns only while online (+ instant check on join), orphan-boss cleanup on enable, broadcast = broadcast-only + 60s cd. Old spawn-interval-minutes/max-alive retired (keys kept, unread).
- **MED-05 Mail:** full mailbox silently DELETED oldest (griefable). Now REJECTS the send at 100 unclaimed before anything is taken (user: 100 unclaimed = hoarding, blocking is correct).
- **MED-06 Locks:** hoppers drained + pistons pushed locked blocks. Added InventoryMoveItemEvent + piston extend/retract guards (config protect-hopper-piston, default true).
- **MED-07 teleports:** /couple home|tp + /guild home were instant. Now 5s stand-still + 12-block monster check (TPA pattern; /couple tp resolves partner at fire time).
- **MED-03/04 AH:** user decision = KEEP full-inbox destruction (hoarder's fault) but never silent (explicit DESTROYED messages on cancel + expiry). Escrow refunds got a pending fallback (refundBidder helper) so money can't vanish.
